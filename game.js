...
const COLOR = {
  top:   { name:'Жёлтые',  key:'yellow', hex:'#f0b02b', dark:'#a06f12', icon:'🟡' },
  right: { name:'Синие',   key:'blue', hex:'#4a92d6', dark:'#1d5c94', icon:'🔵' },
  bottom:{ name:'Красные',  key:'red', hex:'#e0524c', dark:'#8e1f1c', icon:'🔴' },
  left:  { name:'Зелёные', key:'green', hex:'#57a05e', dark:'#2c6234', icon:'🟢' },
};
...
function showScreen(name){
  ...
  var _gcb=document.getElementById('gameChatBtn'); if(_gcb) _gcb.style.display = 'none';
...
var IMG={}, IMG_READY={};
function _li(k,src){ try{ var im=new Image(); im.onload=function(){IMG_READY[k]=true; try{draw();}catch(e){}}; im.src=src; IMG[k]=im; }catch(e){} }
['red','yellow','blue','green'].forEach(function(k){ _li('pawn_'+k,'assets/pawn_'+k+'.png'); });
_li('wood_light','assets/wood_light.jpg');
_li('wood_dark','assets/wood_dark.jpg');
_li('nameplate','assets/nameplate.png');
var _pat={};
function woodPat(k){ if(!IMG_READY[k]) return null; if(_pat[k]) return _pat[k]; try{ _pat[k]=ctx.createPattern(IMG[k],'repeat'); }catch(e){ _pat[k]=null; } return _pat[k]; }
let cellSize = 10;

if(R3D && window.Board3D){
  try{
    Board3D.init(R3D, {
      N:N, L:L, W:W, C:C, HOME_LEN:HOME_LEN,
      COLOR:COLOR, DIRS:DIRS, BOARD:BOARD, BASE_SLOTS:BASE_SLOTS, CORNER_CELLS:CORNER_CELLS,
      stepToCell:stepToCell, pieceCell:pieceCell,
      players:function(){ return players; },
      animState:function(){ return animState; },
      bonuses:function(){ try{ return bonuses; }catch(e){ return []; } },
      blockades:function(){ try{ return blockades; }catch(e){ return []; } },
      particles:function(){ return particles; },
      shocks:function(){ return shocks; },
      trails:function(){ return trails; },
      emotes:function(){ try{ return emotes; }catch(e){ return []; } },
      mover:function(){ return mover(); },
      activePlayer:function(){ return activePlayer(); },
      selectedPiece:function(){ return selectedPiece; },
      validTargets:function(){ return validTargets; },
      movablePieces:function(){ return movablePieces; },
      mustPickPiece:function(){ return mustPickPiece; },
      gameMode:function(){ return gameMode; },
      gameOver:function(){ return gameOver; },
      isCrazy:function(){ try{ return isCrazy(); }catch(e){ return false; } },
      diceValue:function(){ return diceValue; },
      cellSize:function(){ return cellSize; },
      getHomeCount:function(m){ return getHomeCount(m); },
      winTarget:function(){ return winTarget(); },
      shieldSecondsLeft:function(pc){ try{ return shieldSecondsLeft(pc); }catch(e){ return 0; } },
      pieceColorFor:function(d){ return pieceColorFor(d); },
      profAvatar:function(){ try{ return profAvatar(); }catch(e){ return null; } },
      IMG:function(){ return IMG; },
      IMG_READY:function(){ return IMG_READY; },
      getPhysicalTargetsForPiece:function(m,i){ return getPhysicalTargetsForPiece(m,i); }
    });
  }catch(e){ try{ console.warn('Board3D init failed', e); }catch(_){} }
}
...
function resizeCanvas(){
  ...
  if(R3D){
    ...
    R3D.resize(size, size, dpr);
    canvas.style.width = size+'px';
    canvas.style.height = size+'px';
    cellSize = canvas.width / N;
    if(window.Board3D) Board3D.onResize(size);
  } else {
...
function draw(){
  if(R3D && window.Board3D && Board3D.ready){ try{ Board3D.render(); return; }catch(e){ if(!draw._err){ draw._err=1; try{ console.error('Board3D render error', e); }catch(_){} } } }
  ...
  drawEffects(now);
  drawEmotes(now);
  try{ drawOnlineLabels(); }catch(e){}
  /* HUD-uglы убраны: давали лишние голубые рамки у баз */
}

function drawOnlineLabels(){
  if(!players || !players.length) return;
  var cxC = canvas.width/2, cyC = canvas.height/2;
  ctx.save();
  ctx.textBaseline='middle';
  players.forEach(function(p){
    var dir=p.dir; var slots=BASE_SLOTS[dir]; if(!slots||!slots.length) return;
    var sx=0, sy=0;
    slots.forEach(function(rc){ var q=px(rc[0],rc[1]); sx+=q.x+cellSize/2; sy+=q.y+cellSize/2; });
    sx/=slots.length; sy/=slots.length;
    var dx=sx-cxC, dy=sy-cyC, dl=Math.sqrt(dx*dx+dy*dy)||1;
    var off=cellSize*0.6;
    var lx=sx+(dx/dl)*off, ly=sy+(dy/dl)*off;
    var col=COLOR[dir];
    var name, av, lvl=1;
    if(NET.on){
      name=(NET.seatNames&&NET.seatNames[dir])?NET.seatNames[dir]:(p.name||col.name);
      var sm=(NET.seatsMeta&&NET.seatsMeta[dir])||null;
      av=(sm&&sm.avatar)?sm.avatar:col.icon;
      lvl=(sm&&sm.level)?sm.level:1;
      if(dir===NET.mySeat){ try{ av=profAvatar(); lvl=levelInfo(PROF.xp).level; }catch(e){} }
    } else {
      if(p.isAI){ name=(p.name||col.name); av=col.icon; lvl=1; }
      else { name=(function(){ try{ return profNick()||col.name; }catch(e){ return col.name; } })(); try{ av=profAvatar(); lvl=levelInfo(PROF.xp).level; }catch(e){ av=col.icon; } }
    }
    if(name&&name.length>10) name=name.slice(0,9)+'…';
    var emojiFont=Math.max(16,Math.round(cellSize*0.66));
    var nameFont=Math.max(12,Math.round(cellSize*0.46));
    var lvlFont=Math.max(10,Math.round(cellSize*0.34));
    ctx.font='800 '+nameFont+'px system-ui,-apple-system,sans-serif';
    var tw=ctx.measureText(name).width;
    var lvlText='Lv '+lvl;
    ctx.font='700 '+lvlFont+'px system-ui,-apple-system,sans-serif';
    var lw=ctx.measureText(lvlText).width;
    var padX=cellSize*0.26, gap=cellSize*0.16;
    var textW=Math.max(tw,lw);
    var boxW=padX*2+emojiFont+gap+textW;
    var boxH=Math.max(cellSize*1.05, emojiFont+cellSize*0.34);
    var bx=lx-boxW/2, by=ly-boxH/2;
    bx=Math.max(3, Math.min(bx, canvas.width-boxW-3));
    by=Math.max(3, Math.min(by, canvas.height-boxH-3));
    LABEL_POS[dir]={x:bx,y:by,w:boxW,h:boxH};
    var cy2=by+boxH/2;
    var isTurn=(typeof mover==="function") && mover() && mover().dir===dir && !gameOver;
    var _np=IMG['nameplate'];
    if(_np && IMG_READY['nameplate']){
      ctx.save();
      if(isTurn){ ctx.shadowColor='#ffd54a'; ctx.shadowBlur=Math.max(6,cellSize*0.5); }
      ctx.drawImage(_np, bx-boxW*0.14, by-boxH*0.30, boxW*1.28, boxH*1.62);
      ctx.restore();
    } else {
      var wg=ctx.createLinearGradient(bx,by,bx,by+boxH);
      wg.addColorStop(0,'#a9752f'); wg.addColorStop(1,'#734d1e');
      ctx.fillStyle=wg;
      roundRect(bx,by,boxW,boxH,boxH*0.28); ctx.fill();
      ctx.lineWidth=Math.max(2,cellSize*(isTurn?0.13:0.06));
      ctx.strokeStyle=isTurn?'#ffd54a':'#5e3d15';
      ctx.stroke();
    }
    ctx.beginPath();
    ctx.arc(bx+padX+emojiFont*0.42, cy2, emojiFont*0.62, 0, 7);
    ctx.fillStyle=col.hex+'33'; ctx.fill();
    ctx.lineWidth=Math.max(1.5,cellSize*0.05); ctx.strokeStyle=col.hex; ctx.stroke();
    ctx.textAlign='center';
    ctx.font=emojiFont+'px system-ui,-apple-system,sans-serif';
    ctx.fillStyle='#fff';
    ctx.fillText(av, bx+padX+emojiFont*0.42, cy2+1);
    ctx.textAlign='left';
    var tx=bx+padX+emojiFont+gap;
    ctx.fillStyle='#fff4e0';
    ctx.font='800 '+nameFont+'px system-ui,-apple-system,sans-serif';
    ctx.fillText(name, tx, by+boxH*0.36);
    ctx.fillStyle=isTurn?'#ffd54a':'#f0d9a8';
    ctx.font='700 '+lvlFont+'px system-ui,-apple-system,sans-serif';
    ctx.fillText(lvlText, tx, by+boxH*0.68);
    if(isTurn){
      ctx.beginPath(); ctx.arc(bx+boxW-boxH*0.24, cy2, Math.max(3,cellSize*0.09), 0, 7); ctx.fillStyle='#ffd54a'; ctx.fill();
    }
  });
  ctx.restore();
  try{ drawPlayerEmotes((typeof performance!=='undefined'&&performance.now)?performance.now():Date.now()); }catch(e){}
}
...
function drawPiece(cx,cy,rad,col,highlight,selected,pulse){
  ...
  if(img && IMG_READY['pawn_'+key]){
    ...
    ctx.save();
    ctx.fillStyle='rgba(18,10,3,0.30)';
    ctx.beginPath(); ctx.ellipse(cx, cy+rad*0.82, rad*0.82, rad*0.30, 0,0,7); ctx.fill();
    if(col&&col.anim==='glow'){ ctx.shadowColor=col.hex; ctx.shadowBlur=rad*(0.7+0.7*(0.5+0.5*Math.sin(_now/280))); }
    if(highlight||selected){ ctx.shadowColor=selected?'#ffffff':col.hex; ctx.shadowBlur=rad*(1.1+0.8*pulse); }
    ctx.drawImage(img, dx, dy, pw, ph);
    ctx.restore();
    if(highlight||selected){
      ctx.save();
      ctx.beginPath(); ctx.ellipse(cx, cy+rad*0.7, rad*(1.0+0.12*pulse), rad*(0.42+0.06*pulse), 0,0,7);
      ctx.strokeStyle=selected?'#ffffff':col.hex; ctx.lineWidth=Math.max(2,rad*0.14);
      ctx.setLineDash([rad*0.42,rad*0.32]); ctx.stroke(); ctx.restore();
    }
    return;
  }
...
function setDiceRotation(value){
  if(R3D && window.Board3D && Board3D.ready){ try{ Board3D.setValue(value); }catch(e){} }
  const r = ROT[value];
  if(typeof dice3d !== 'undefined' && dice3d){ dice3d.style.transform = `rotateX(${r.x + TILT_X}deg) rotateY(${r.y + TILT_Y}deg)`; }
}
setDiceRotation(1);
...
canvas.addEventListener('pointerdown', (e)=>{
  ...
  const use3D = !!(R3D && window.Board3D && Board3D.ready);
  let mx, my, cellPx;
  if(use3D){ mx = e.clientX-rect.left; my = e.clientY-rect.top; cellPx = rect.width/N; }
  else { const scaleX = canvas.width/rect.width, scaleY = canvas.height/rect.height; mx=(e.clientX-rect.left)*scaleX; my=(e.clientY-rect.top)*scaleY; cellPx = cellSize; }
  const cellCenter = (r,c)=>{ if(use3D){ const s=Board3D.screenForCell(r,c); return {cx:s.x, cy:s.y}; } const p=px(r,c); return {cx:p.x+cellSize/2, cy:p.y+cellSize/2}; };
  ...
  if(gameMode === 'physical'){
    if(selectedPiece >= 0 && validTargets.length > 0){
      ...
      let bestD = 1e9;
      validTargets.forEach(tgt => {
        const {cx, cy} = cellCenter(tgt.cell[0], tgt.cell[1]);
        const d = Math.hypot(mx - cx, my - cy);
        if(d < cellPx * 0.95 && d < bestD){
          bestD = d;
          hitTarget = tgt;
        }
      });
  ...
  movablePieces.forEach(i=>{
    ...
    const {cx,cy}=cellCenter(r,c);
    const d = Math.hypot(mx-cx,my-cy);
    if(d<cellPx*1.0 && d<bestD){ bestD=d; best=i; }
  });
  if(best>=0){
...
function tintDice(dir){
  if(!diceBtn) return;
  var base = COLOR[dir];
  var pc = null; try{ if(typeof pieceColorFor==="function") pc = pieceColorFor(dir); }catch(e){}
  var col = pc || base;
  if(!col) return;
  var hex = col.hex || (base && base.hex);
  var dark = col.dark || (base && base.dark) || hex;
  diceBtn.style.setProperty('--dcol', hex);
  diceBtn.style.setProperty('--dcol-l', lighten(hex, 0.45));
  diceBtn.style.setProperty('--dcol-d', dark);
  diceBtn.style.boxShadow = 'inset 0 1px 1px rgba(255,255,255,0.2), 0 12px 28px rgba(0,0,0,0.55), 0 0 20px ' + hex + '55';
  try{ var root=document.documentElement; root.style.setProperty('--dcol', hex); root.style.setProperty('--dcol-l', lighten(hex,0.45)); root.style.setProperty('--dcol-d', dark); }catch(e){}
}
...
function rollDice(power, vx, vy){
  ...
  if(R3D && window.Board3D && Board3D.ready){
    const _mode = (Settings.throwMode === 'instant') ? 'instant' : ((!Settings.fx || Settings.throwMode === 'fast') ? 'fast' : 'physics');
    if(_mode === 'instant'){ Sound.hop(); Haptic.land(); }
    else { Sound.roll(); Haptic.roll(pw); }
    Board3D.rollDice(base, _mode, pw, function(){ if(_mode!=='instant'){ Sound.hop(); Haptic.land(); } finish(); });
    return;
  }
...
(function wireProfSheets(){
  var mk=function(btnId, sheetId, fn){ var b=document.getElementById(btnId); if(!b) return; addTapListener(b, function(){ try{ if(fn) fn(); }catch(e){} openSheet(sheetId); }); };
  mk('openShopBtn','shopSheet', function(){ shopRender(); });
  mk('openAchBtn','achSheet', function(){ achRender(); });
  mk('openFriendsBtn','friendsSheet', function(){ friendsWire(); friendsRender(); });
  mk('openWorldChatBtn','worldChatSheet', null);
  mk('openLeaderBtn','leaderSheet', function(){ try{ netLoadLeaderboard(); }catch(e){} });
})();
...
function emoteEvent(kind, p, i, noEmoji){
  if(!noEmoji && p){ try{ pushPlayerEmote(p.dir, EMO_ICON[kind]); }catch(e){} }
...
var playerEmotes={};
var LABEL_POS={};
function pushPlayerEmote(dir, emoji){ if(!dir||!emoji) return; playerEmotes[dir]={emoji:emoji,start:(typeof performance!=='undefined'&&performance.now)?performance.now():Date.now()}; }
function drawPlayerEmotes(now){
  try{
    var keys=Object.keys(playerEmotes); if(!keys.length) return;
    keys.forEach(function(dir){
      var em=playerEmotes[dir]; if(!em) return;
      var t=(now-em.start)/2200;
      if(t>=1){ delete playerEmotes[dir]; return; }
      var pos=LABEL_POS[dir]; if(!pos) return;
      ctx.save();
      ctx.globalAlpha = t<0.1 ? t/0.1 : Math.max(0,1-(t-0.1)/0.9);
      var bs=Math.max(26,cellSize*1.15);
      var bw=bs*1.35, bh=bs;
      var by;
      if(pos.y < canvas.height*0.5){ by=pos.y+pos.h+4; } else { by=pos.y-bh-4; }
      var bxL=Math.max(2,Math.min(pos.x+pos.w*0.5-bw/2, canvas.width-bw-2));
      var wg=ctx.createLinearGradient(bxL,by,bxL,by+bh);
      wg.addColorStop(0,'rgba(250,240,215,0.98)'); wg.addColorStop(1,'rgba(236,214,170,0.98)');
      ctx.fillStyle=wg;
      roundRect(bxL,by,bw,bh,bh*0.32); ctx.fill();
      ctx.strokeStyle='#7c5220'; ctx.lineWidth=Math.max(1.5,cellSize*0.05); ctx.stroke();
      ctx.font=(bs*0.6)+'px sans-serif'; ctx.textAlign='center'; ctx.textBaseline='middle';
      ctx.fillText(em.emoji, bxL+bw/2, by+bh/2+1);
      ctx.restore();
    });
  }catch(e){}
}
...
function netUid(){
  try{ if(typeof PROF!=='undefined' && PROF.uid) return PROF.uid; }catch(e){}
  var id=null;
...
function netCreateRoom(){
  if(!PROF||!PROF.uid){ onlineMsg('Сначала войди в профиль (кнопка «Профиль» в меню)'); return; }
...
function renderLobbySeats(room){
  ...
  seatDirs.forEach(function(d,i){
    ...
    row.innerHTML='<div class="slot-name"><span class="swatch" style="background:'+col.hex+';color:'+col.hex+'"></span>'+col.icon+' '+col.name+'</div>'
      +'<div style="font-size:12px;font-weight:700;color:'+(s?col.hex:'#6b7f96')+'">'+(s?(((s.avatar?s.avatar+' ':'')+(s.name||('Игрок '+(i+1)))+(isHost?' 👑':''))+(you?' · ты':'')):'ожидание…')+'</div>';
...
function profMsg(t){ var e=document.getElementById('profMsg'); if(e) e.textContent=t||''; }
function profMsg2(t){ var e=document.getElementById('profMsg2'); if(e) e.textContent=t||''; }
function profNick(){ try{ if(PROF && PROF.nick) return PROF.nick; }catch(e){} return null; }
function profAvatar(){ try{ if(PROF && PROF.avatar) return PROF.avatar; }catch(e){} return '🐵'; }

function profAuthReady(){
  if(_fbAuth) return _fbAuth;
  if(!netInit()) return null;
  try{ _fbAuth=firebase.auth(); }catch(e){ console.error('auth init', e); return null; }
  return _fbAuth;
}
...
function profInit(){
  var a=profAuthReady();
  if(!a) return;
  try{ a.getRedirectResult().catch(function(e){ if(e&&e.message) profMsg('Google: '+e.message); }); }catch(e){}
  a.onAuthStateChanged(function(user){
    if(user){ profOnSignedIn(user); }
    else { PROF.uid=null; PROF.ready=false; profRender(); }
  });
}

function profDefaultNick(user){
  if(user && user.displayName){ return String(user.displayName).slice(0,20); }
  return 'Игрок'+Math.floor(1000+Math.random()*9000);
}

function profOnSignedIn(user){
  PROF.uid=user.uid;
  PROF.isGoogle=(user.providerData && user.providerData.some(function(p){ return p.providerId==='google.com'; }));
  var ref=_fbDb.ref('profiles/'+user.uid);
  profMsg('Загрузка профиля…');
  ref.get().then(function(snap){
    if(snap.exists()){
      var d=snap.val();
      PROF.nick=d.nick||profDefaultNick(user);
      PROF.avatar=d.avatar||'🐵';
...
function profSave(){
  if(!PROF.uid || !_fbDb) return;
  try{
    _fbDb.ref('profiles/'+PROF.uid).update({
  ...
  }catch(e){ console.error('profSave', e); }
}

function profAwardCoins(n, reason){
  if(!PROF.uid) return;
  PROF.coins=(PROF.coins||0)+n; profSave(); profRender();
}

function profSignInGuest(){
  var a=profAuthReady(); if(!a){ profMsg('⚠ Firebase не настроен'); return; }
  profMsg('Вход…');
  a.signInAnonymously().catch(function(e){
    if(e && e.code==='auth/operation-not-allowed') profMsg('⚠ Включи Anonymous в Firebase → Authentication');
    else profMsg('Ошибка входа: '+e.message);
  });
}
function profSignInGoogle(){
  var a=profAuthReady(); if(!a){ profMsg('⚠ Firebase не настроен'); return; }
  var provider=new firebase.auth.GoogleAuthProvider();
  profMsg('Вход через Google…');
  a.signInWithPopup(provider).catch(function(e){
    if(e && (e.code==='auth/popup-blocked'||e.code==='auth/operation-not-supported-in-this-environment'||e.code==='auth/cancelled-popup-request'||e.code==='auth/popup-closed-by-user')){
      try{ a.signInWithRedirect(provider); }catch(e2){ profMsg('Ошибка: '+e2.message); }
    } else if(e && e.code==='auth/operation-not-allowed'){ profMsg('⚠ Включи Google в Firebase → Authentication'); }
    else if(e && e.code==='auth/unauthorized-domain'){ profMsg('⚠ Добавь домен сайта в Firebase → Authentication → Settings → Authorized domains'); }
    else { profMsg('Ошибка входа: '+e.message); }
  });
}
function profSignOut(){
  var a=profAuthReady(); if(!a) return;
  a.signOut().then(function(){ PROF.uid=null; PROF.ready=false; profRender(); });
}

function profSaveNick(){
  var inp=document.getElementById('profNickInput');
  var v=((inp&&inp.value)||'').trim().slice(0,20);
  if(v.length<1){ profMsg2('Ник не может быть пустым'); return; }
  PROF.nick=v; profSave(); profRender(); profMsg2('✓ Ник сохранён');
}
function profSetAvatar(a){ PROF.avatar=a; profSave(); profRender(); }

function profBuildAvatarGrid(){
  var grid=document.getElementById('profAvatarGrid'); if(!grid) return;
  grid.innerHTML='';
  PROF_AVATARS.forEach(function(a){
    var b=document.createElement('button');
    b.type='button'; b.textContent=a;
    b.style.cssText='font-size:24px;width:44px;height:44px;border-radius:10px;border:1px solid '+(a===PROF.avatar?'#ffd54a':'#2a3b52')+';background:'+(a===PROF.avatar?'#2a2410':'#0b1524')+';cursor:pointer;';
    b.addEventListener('click', function(){ profSetAvatar(a); });
    grid.appendChild(b);
  });
}

function profRender(){
  var auth=document.getElementById('profAuth');
  var card=document.getElementById('profCard');
  if(!auth||!card) return;
  if(!PROF.ready || !PROF.uid){ auth.style.display='block'; card.style.display='none'; return; }
  auth.style.display='none'; card.style.display='block';
...
(function wireProfile(){
  var q=function(id){ return document.getElementById(id); };
  var back=q('profBack'); if(back) back.addEventListener('click', function(){ showScreen('menu'); });
  var g=q('profGuestBtn'); if(g) g.addEventListener('click', profSignInGuest);
  var gg=q('profGoogleBtn'); if(gg) gg.addEventListener('click', profSignInGoogle);
  var so=q('profSignOut'); if(so) so.addEventListener('click', profSignOut);
...
function applyDiceSkin(){
  ...
  try{ if(typeof players!=="undefined" && players && players.length && !gameOver && typeof mover==="function"){ var _mv=mover(); if(_mv) tintDice(_mv.dir); } }catch(e){}
}
...
function reactShow(m){
  try{
    var emoji=m.emoji||'\ud83d\udc4d';
    var dir=m.dir||null;
    if(dir){ try{ pushPlayerEmote(dir, emoji); return; }catch(e){} }
    var who=m.name||'';
    var t=document.createElement('div');
    t.textContent=emoji+(who?(' '+who):'');
    t.style.cssText='position:fixed;left:50%;bottom:120px;transform:translateX(-50%) scale(.6);z-index:9400;font-size:28px;background:rgba(60,38,14,.9);border:1px solid #7c5220;border-radius:20px;padding:6px 14px;color:#fff4e0;pointer-events:none;transition:all 1.1s cubic-bezier(.2,.9,.3,1);opacity:1;white-space:nowrap;';
    document.body.appendChild(t);
